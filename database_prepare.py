...
parser.add_argument('--image_path', default='./image_database', type=str, help='|人脸数据库图片位置|')
...
def database_prepare():
    ...
    image_dir = sorted(os.listdir(args.image_path))  # 获取图片路径
    ...
    df_database.to_csv(args.save_path, index=False)  # 将特征保存至csv文件
